...
public class DeleteSelectedReservationAction implements ActionListener {

	private static final String CONFIRM_DELETE_TITLE = "Confirm deletion";
	private static final String NO_SELECTION_MSG = "Selecciona al menos una reservation para borrar.";
	private static final String PARTIAL_SUCCESS_MSG = "Algunas reservations no se pudieron borrar.";
	private static final String SUCCESS_MSG = "Reservations eliminadas correctamente.";
	private static final String ERROR_MSG = "Error borrando reservations: ";
	...
	private final Component parent;
        ...
        public DeleteSelectedReservationAction(Supplier<List<Integer>> idsSupplier, Component parent, ReservationService service,
	...
	private boolean confirmDelete(int count) {
		String message = String.format("Delete %d reservation%s seleccionada%s?", count, count > 1 ? "s" : "",
				count > 1 ? "s" : "");
	...
	private void performDelete(List<Integer> ids) {
		try {
			DeleteResult result = deleteReservations(ids);
        ...
        private DeleteResult deleteReservations(List<Integer> ids) throws RentexpresException {
	...
	private void showResultMessage(DeleteResult result) {
		if (result.totalCount == result.successCount) {
			...
			SwingUtils.showError(parent, "No se pudo delete ninguna reservation.");
		}
	}
